...
import { 
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, ReferenceLine, Cell, TooltipProps
} from 'recharts';
...
const formatXAxis = (tickItem: number | string) => {
  const num = Math.abs(Number(tickItem));
...
export default function DemographicPyramid({ 
  ...
  const [dados, setDados] = useState<PiramideData[]>([]);
